...
class CachedModelResult(BaseModel):
    ...
    def add_run(self, result: EvaluationResult) -> None:
...
class EvaluationCache(BaseModel):
    ...
    def print_summary(self) -> str:
        ...
        if rankings:
            ...
            lines.append("-" * 70)
            lines.append(f"{'Rank':<5} {'Model':<40} {'Avg Score':<12} {'Runs':<6}")
            lines.append("-" * 70)
            
            for i, (key, avg_score, run_count) in enumerate(rankings, 1):
                lines.append(f"{i:<5} {key:<40} {avg_score*100:>6.1f}%      {run_count:<6}")
